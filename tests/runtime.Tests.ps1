...
Describe "[runtime > $TEST_TAG] build image" {
  BeforeEach {
...
Describe "[runtime > $TEST_TAG] cleanup container" {
  It 'cleanup' {
    Cleanup $SUT_CONTAINER | Out-Null
  }
}

Describe "[runtime > $TEST_TAG] test multiple JENKINS_OPTS" {
...
Describe "[runtime > $TEST_TAG] test jenkins arguments" {
...
Describe "[runtime > $TEST_TAG] passing JVM parameters" {
  BeforeAll {
