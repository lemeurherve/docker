...
Describe "[runtime/$global:TEST_TAG] build image" {
  ...
  It 'builds image' {
    $exitCode, $stdout, $stderr = Run-Program 'docker.exe' "build --build-arg JENKINS_VERSION=$env:JENKINS_VERSION --build-arg JENKINS_SHA=$env:JENKINS_SHA --build-arg COMMIT_SHA=$env:COMMIT_SHA --file $env:DOCKERFILE -t $global:SUT_IMAGE ."
