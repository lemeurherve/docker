...
Describe "[plugins-cli > $TEST_TAG] build image" {
  BeforeEach {
    Push-Location -StackName 'jenkins' -Path "$PSScriptRoot/.."
  }

  It 'builds image' {
    $exitCode, $stdout, $stderr = Build-Docker $SUT_IMAGE
...
Describe "[plugins-cli > $TEST_TAG] cleanup container" {
  It 'cleanup' {
    Cleanup $SUT_CONTAINER | Out-Null
  }
}

Describe "[plugins-cli > $TEST_TAG] plugins are installed with jenkins-plugin-cli" {
...
Describe "[plugins-cli > $TEST_TAG] plugins are installed with jenkins-plugin-cli with non-default REF" {
  It 'builds child image' {
...
Describe "[plugins-cli > $TEST_TAG] plugins are installed with jenkins-plugin-cli from a plugins file" {
  It 'builds child image' {
    $exitCode, $stdout, $stderr = Build-DockerChild $SUT_IMAGE-plugins-cli $PSScriptRoot/plugins-cli
    $exitCode | Should -Be 0
  }
...
Describe "[plugins-cli > $TEST_TAG] plugins are installed with jenkins-plugin-cli even when already exist" {
  It 'builds child image' {
    $exitCode, $stdout, $stderr = Build-DockerChild $SUT_IMAGE-plugins-cli $PSScriptRoot/plugins-cli
    $exitCode | Should -Be 0
  }
...
Describe "[plugins-cli > $TEST_TAG] clean work directory" {
  It 'cleanup' {
...
Describe "[plugins-cli > $TEST_TAG] plugins are getting upgraded but not downgraded" {
  It 'builds child image' {
    # Initial execution
    $exitCode, $stdout, $stderr = Build-DockerChild $SUT_IMAGE-plugins-cli $PSScriptRoot/plugins-cli
    $exitCode | Should -Be 0
...
Describe "[plugins-cli > $TEST_TAG] do not upgrade if plugin has been manually updated" {
...
Describe "[plugins-cli > $TEST_TAG] upgrade plugin even if it has been manually updated when PLUGINS_FORCE_UPGRADE=true" {
  It 'builds child image' {
    $exitCode, $stdout, $stderr = Build-DockerChild $SUT_IMAGE-plugins-cli $PSScriptRoot/plugins-cli
    $exitCode | Should -Be 0
  }
...
Describe "[plugins-cli > $TEST_TAG] plugins are installed with jenkins-plugin-cli and no war" {
  It 'builds child image' {
...
Describe "[plugins-cli > $TEST_TAG] Use a custom jenkins.war" {
  It 'builds child image' {
